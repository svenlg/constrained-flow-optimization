...
def step(model, adj, g_t, t, alpha, alpha_dot, dt, upper_edge_mask, calc_adj=True):
    ...
    with torch.enable_grad():
        ...
        for k, feat in enumerate(['x', 'a', 'c', 'e']):
            ...
            factor = alpha_dot[k] / (1 - alpha[k])
            factor = torch.where(torch.isfinite(factor), factor, torch.zeros_like(factor))
            factor = torch.clamp(factor, max=50.0)

            v_pred[feat] = factor * (x_1 - x_t)

            if calc_adj:
                eps_pred = 2 * v_pred[feat] - alpha_dot[k]/(alpha[k]+dt) * x_t
...
class LeanAdjointSolverFlow:
    ...
    def solve(self, graph_trajectories, ts):
        ...
        res = {
            ...
            'traj_adj': trajs_adj[:-1], # list of dicts with {x, a, c, e} each (T,)
            'traj_v_base': traj_v_base[1:], # list of dicts with {x, a, c, e} each (T,)
        }
