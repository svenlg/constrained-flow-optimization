...
class AugmentedLagrangian:
    ...
    def expected_constraint(self, new_samples):
        """
        Compute E[constraint_fn(samples)] in a memory-safe (batched) way.
        """
        total = []
        count = []
        for i in range(0, len(new_samples), MAX_GPU_SAMPLES):
            if i + MAX_GPU_SAMPLES > len(new_samples):
                tmp_batch = dgl.batch(new_samples[i:]).to(self.device)
                count.append(len(new_samples) - i)
            else:
                tmp_batch = dgl.batch(new_samples[i:i + MAX_GPU_SAMPLES]).to(self.device)
                count.append(MAX_GPU_SAMPLES)
            with torch.no_grad():
                vals = self.constraint_fn(tmp_batch).mean().detach().cpu().item()
                total.append(float(vals))

        self.exp_constraint = sum(np.array(total) * np.array(count)) / sum(np.array(count))
