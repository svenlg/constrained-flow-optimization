...
def sampling(
    ...
    if n_atoms_provided:
        if (not isinstance(n_atoms, int) or (n_atoms < MIN_ALLOWED_ATOMS) or (n_atoms > MAX_ALLOWED_ATOMS)):
        ...
        if batch_size is None:
            batch_size = MAX_GPU_SAMPLES

        for start in range(0, num_samples, batch_size):
            bs = min(batch_size, num_samples - start)

            tmp_molecules = model.sample(
                n_atoms=torch.tensor([n_atoms] * bs, device=device),
                n_timesteps=config.num_integration_steps + 1,
                device=device,
            )
            new_molecules.extend(tmp_molecules)
            del tmp_molecules

        # Sanity Check
        assert len(new_molecules) == num_samples, (len(new_molecules), num_samples)
    ...
    for mol in new_molecules:
        dgl_mols.append(copy.deepcopy(mol.g.cpu()))
