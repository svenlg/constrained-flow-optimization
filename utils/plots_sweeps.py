...
def style_for_name(name):
    ...
    # return random color for others
...
def plot_sweep(
    ...
    last_intersection_point_only=False,
):
    ...
    with plt.rc_context(rc):
        ...
        if k_update_steps is not None and k_update_steps > 0:
            ...
            if last_intersection_point_only:
                print("Only plotting last intersection point of CFO curve and vertical bars.")
                vlines_x = [vlines_x[-1]]

            for ax in (ax_reward, ax_constraint):
                for xv in vlines_x:
                    print(f"Plotting vertical line at x={xv}")
        ...
        xlabel = r'$K$' if (use_k_axis and k_update_steps is not None) else "N"
...
def main():
    ...
    parser.add_argument("--base_line_comparison", action="store_true",
                        help="Use varied baseline colors for different baseline values.")
    ...
    parser.add_argument("--last_intersection_point_only", action="store_true",
                        help="Only plot the last intersection point of CFO curve and vertical bars.")
    ...
    if args.cfo_dir is not None and args.baseline_dir is not None:
        ...
        plot_sweep(
            ...
            base_line_comparison=args.base_line_comparison,
            ...
            last_intersection_point_only=args.last_intersection_point_only
        )
